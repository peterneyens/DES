...
public class Feistel {
    ...
    public byte[] executeFunction(byte[] block, byte[] subkey) throws Exception {
        ...
        byte[] result = substitution6x4(xoredBytes);
//        byte[] result = executeS(xoredBytes);
    ...
    private static byte[] splitBytes(byte[] in, int len) {
        ...
        for (int i = 0; i < numOfBytes; i++) {
            for (int j = 0; j < len; j++) {
                int val = ByteHelper.getBit(in, len * i + j);
                ByteHelper.setBit(out, 8 * i + j, val);
            }
        }
        return out;
    }
    ...
    private byte[] executeS(byte[] block) {
        ...
        for (boolean isBitSet : bitStream) {
            ...
            if (count % 6 == 0 && count != 0) // increment for every 6 bits
            {
                byteIndex++;
            }
